...
const categoriaConfig: Record<string, { icon: typeof ShieldAlert; color: string }> = {
  "Possível Fraude": { icon: ShieldAlert, color: "bg-destructive/10 text-destructive" },
  "Inconsistência Documental": { icon: AlertCircle, color: "bg-destructive/10 text-destructive" },
  "Dano Oculto Detectado": { icon: Zap, color: "bg-orange-500/10 text-orange-600" },
...
export function AlertasFeed({ alertas, onMarcarLido, onIrParaAnalise }: AlertasFeedProps) {
  if (alertas.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <CheckCircle className="h-12 w-12 text-muted-foreground/50 mb-4" />
        <h3 className="text-lg font-medium text-foreground mb-1">Nenhum alerta encontrado</h3>
  ...
  return (
    <div className="space-y-3">
      {alertas.map((alerta) => {
        ...
        const catConfig = categoriaConfig[alerta.categoria] || { icon: AlertCircle, color: "bg-muted text-muted-foreground" }
